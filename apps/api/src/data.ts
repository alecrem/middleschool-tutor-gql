...
export function searchCards(query: string, limit: number = 50): { cards: MagicCard[], total: number } {
  ...
  return cards.filter(card => {
    return colors.every(color => {
      switch (color.toLowerCase()) {
        ...
        case 'u': return card.u
        case 'b': return card.b
        case 'r': return card.r
        case 'g': return card.g
        case 'c': return card.c
